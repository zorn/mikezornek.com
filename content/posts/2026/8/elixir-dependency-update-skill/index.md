...
If you have experimented with AI coding tools, or are considering it, one feature of the coding harness (the interactive shell you work in) you'll run into is skills.
...
One frustration (which better configuration can resolve) is that I'd prefer that Dependabot group the updates together. I see no need to update [Oban](https://hex.pm/packages/oban) and [Oban Web](https://hex.pm/packages/oban_web) in separate PRs, for example. I also don't need to see this every day; limiting it to weekly, or even monthly for quiet side projects, is preferred and seems like a better balance of signal to noise.
...
Here are output examples from my [Flick](https://github.com/zorn/flick/pull/184) and [LocalCents](https://github.com/zorn/local_cents/pull/193) projects.
...
I also host [a skill on writing skills](https://github.com/zorn/dotfiles/blob/main/claude/skills/writing-skills/SKILL.md). It is a fork of [Matt's skill](https://github.com/mattpocock/skills) but I added changed some things, like [a preference towards US English](https://github.com/zorn/dotfiles/pull/22/commits).

## Read the code

This may feel repetitive if you read my other posts, but having a skill to automate the monotony does not absolve you of reading the code, the changelog, and the diffs yourself. The skill automates the assembly of the PR. You are still on the hook for understanding how these library updates affect your project.
