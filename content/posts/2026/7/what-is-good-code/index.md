...
date: 2026-07-13T19:38:12-04:00
...
I was drafting a few other blog posts about my experiments using an AI-based software lifecycle and the evolution of my CI toolbelt to keep it all in check, when it had occurred to me that I've never really put pencil to paper about what is good code. All those other practices are in service to good code, so let's set some ground rules as to how we consider what is good code and help connect the what to the why.

Q: Should we put "good code" in quotes?

## Solves a problem.

An obvious starting point, but if we are not helping people accomplish some kind of goal than WTF are we doing?

This includes software being deployed and available to users. If you are endlessly [chasing perfection](https://jawns.club/@zorn/116914095006092357) and not shipping than you have lost the plot. In fact, the learning you will do once you can observe users attempting to solve their problem with your code is probably more valuable than the other quality attributes we are about to cover.

Enabling practices: user research, marketing/finding your people, one button deployments, product usage observability, accessibility, localization

## Is accurate, reliable and secure.

When the user hits those buttons, we meet the intents of the system. We do the work correctly and the work happens with good performance characteristics.

We only do what the user needs. We don't burden the user with features or complexities they do not need. Embrace the YAGNI (you aren't going to need it).

We need to treat all user input with care. Losing user content, intent, or progress needs be protected against via fault tolerant system design.

We protect the integrity of the system through industry security norms.

Enabling practices: testing, telemetry, error reporting, customer support, customer education, security audits

## The system can be understood and the code readable
...
If we had to name enemy number one of "good code" it would be complexity. No one intentionally builds a complex system. It happens one small change at a time. You need to ruthlessly fight for simplicity and regularly fight code entropy.  

Enabling practices: automated code format and pattern enforcement, documentation, ubiquitous language, decision documents, code standard definitions
