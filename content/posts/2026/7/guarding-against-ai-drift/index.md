...
**[Credo](https://github.com/rrrene/credo).** A style and consistency linter for Elixir. It ships with a comprehensive set of community-norm defaults (how you organize aliases, name things, structure a pipe), and I run it with [`--strict`](https://github.com/zorn/local_cents/blob/main/.credo.exs) enabled, with most of those defaults on. What makes it powerful is that anyone can write a check, so I layer a few sources on top of the defaults:
...
One tool I'll give a nod to here even though I haven't used it yet: [Paraxial.io](https://paraxial.io/). It's an application-security platform built specifically for Elixir and Phoenix, going beyond the static scanning above to include runtime protection, such as bot and attack detection. LocalCents is currently focused on being a local-first desktop app, so I haven't added it yet, but I'll give it a second look once I start hosting a version on the web.
...
- **Visual-regression testing.** Nothing catches unintended visual changes yet. I'd like to capture baseline screenshots of both my UI component library and key user flows, commit them to the repo, and make any visual diff something I have to approve before it lands. Same idea as everything else here, a locked-in baseline to catch drift, just applied to pixels instead of code.
