...
function expand(ioNode, fully) {
  ...
  if ($("#" + ioNode)[0] != null) {
    ...
    if (nextRow != null) {
      ...
      while (
        nextRow != null &&
        ($(nextRow).attr("data-level") > baseLevel ||
          $(nextRow).hasClass("whole-note"))
      ) {
        if (
          !(
            $(nextRow).hasClass("whole-note") &&
    ...
    $(rowsBeingRevealed)
      .wrapAll(
        '<div class="temp" style="display: none;" id=' + "div" + ioNode + " />",
      )
    ...
    for (var i = 0; i < rowsBeingRevealed.length; i++) {
      ...
      if ($(rowsBeingRevealed[i]).hasClass("whole-note")) {
        if (
          $(rowsBeingRevealed[i]).attr("data-state") == "expanded" &&
          $(rowsBeingRevealed[i]).prev().hasClass("visible")
        ) {
        ...
        // When child rows have children, check for expansion state and set its children accordingly
          ...
          while (foundParent != 1) {
            if (
              $(previousRow).attr("data-level") == currentLevel - 1 &&
              !$(previousRow).hasClass("whole-note")
            ) {
              if (
                $(previousRow).hasClass("visible") &&
                $(previousRow).attr("data-state") == "expanded"
              ) {
                $(rowsBeingRevealed[i])
                  .removeClass("hidden")
                  .addClass("visible");
      ...
      if (
        $(rowsBeingRevealed[i]).next() &&
        $(rowsBeingRevealed[i]).next().attr("data-level") > currentLevel
      ) {
        // Check if previous row is a note, if so we need to look at the note's outline row instead of the note
        if ($(rowsBeingRevealed[i]).hasClass("whole-note")) {
          parentExpansionState = $(rowsBeingRevealed[i])
            .prev()
            .attr("data-state");
    ...
    if (inlineAboveChildrenPadding != "" || inlineBelowChildrenPadding != "") {
      ...
      if (noteRow != null && $(noteRow).hasClass("visible")) {
        if ($(row).attr("data-has-children") == 1) {
          // these children are now visible
          if (inlineRowPadding == "" && inlineAboveChildrenPadding == "") {
            ...
            setBottomPadding(
              noteRow,
              "note",
              getNum(inlineRowPadding) +
                getNum(inlineAboveChildrenPadding) +
                "px",
            );
          }
        }
...
function collapse(ioNode, fully) {
  ...
  if ($("#" + ioNode)[0] != null) {
    ...
    $(rowsToCollapse)
      .wrapAll(
        '<div class="temp" style="display: block;" id=' +
          "div" +
          ioNode +
          " />",
      )
    ...
    $("#div" + ioNode)
      ...
      .always(function () {
        ...
        for (var i = 0; i < rowsToCollapse.length; i++) {
          $(rowsToCollapse[i])
            .removeClass("visible last-child")
            .addClass("hidden");
        }
        // Update alt row colors
        updateRowBackgroundColors();
        ...
        if (inlineRowPadding == "" && inlineBelowChildrenPadding == "") {
          ...
          if ($(noteRow).hasClass("last-child")) {
            setBottomPadding(
              ...
              "note",
              getNum(inlineRowPadding) +
                getNum(inlineBelowChildrenPadding) +
                "px",
            );
          } else {
            if (inlineRowPadding != "") {
...
function expandAll() {
  var allCollapsedParents = $(".whole-row")
    .filter("[data-has-children = '1']")
    .filter("[data-state = 'collapsed']");
...
function collapseAll() {
  ...
  var allLevel1Parents = $(".whole-row")
    .filter("[data-level = '1']")
    .filter("[data-has-children = '1']")
    .filter("[data-state = 'expanded']");
...
function showNote(ioNode) {
  ...
  $("#div" + ioNode).slideDown(150, function () {
    $(".temp-wrapper").contents().unwrap().unwrap();
  });
  ...
  if (inlineAboveNotePadding != "") {
    setBottomPadding(
      noteParentRow,
      "row",
      getNum(inlineAboveNotePadding) + "px",
    );
  }
  if (inlineRowPadding != "") {
  ...
  if ($(noteParentRow).hasClass("last-child")) {
    ...
    if (inlineRowPadding != "" || inlineBelowChildrenPadding != "") {
      setBottomPadding(
        note,
        "note",
        getNum(inlineRowPadding) + getNum(inlineBelowChildrenPadding) + "px",
      );
    }
  } else {
    // Check if above children or row padding needs to be applied
    if (
      $(noteParentRow).attr("data-has-children") == 1 &&
      $(noteParentRow).attr("data-state") == "expanded"
    ) {
      if (inlineRowPadding != "" || inlineAboveChildrenPadding != "") {
        setBottomPadding(
          note,
          "note",
          getNum(inlineRowPadding) + getNum(inlineAboveChildrenPadding) + "px",
        );
      }
    }
  }
...
function hideNote(ioNode) {
  ...
  $(note)
    .wrapAll(
      '<div class="temp" style="display: block;" id=' + "div" + ioNode + " />",
    )
    .wrapAll('<div class="temp-wrapper" style="display: block;" />');

  $("#div" + ioNode).slideUp(150, function () {
    $(".temp-wrapper").contents().unwrap().unwrap();
    $(note)
      .removeClass("visible")
      .addClass("hidden")
...
function setBottomPadding(row, type, paddingValue) {
  if (type == "row") {
    row
      .getElementsByClassName("outline-column")[0]
      .getElementsByTagName("td")[3].style.paddingBottom = paddingValue;
  } else if (type == "note") {
    row
      .getElementsByClassName("note-table")[0]
      .getElementsByTagName("td")[3].style.paddingBottom = paddingValue;
  }
}
...
function setTopPadding(row, type, paddingValue) {
  if (type == "row") {
    row
      .getElementsByClassName("outline-column")[0]
      .getElementsByTagName("td")[3].style.paddingTop = paddingValue;
  } else if (type == "note") {
    row
      .getElementsByClassName("note-table")[0]
      .getElementsByTagName("td")[3].style.paddingTop = paddingValue;
...
function updateRowBackgroundColors() {
  ...
  if (!$(document.body).hasClass("alt-row-colors")) return;
...
function setVisibility() {
  if (document.getElementsByTagName) {
    var allRows = document.querySelectorAll(
      "table.whole-row, table.whole-note",
    );
    ...
    for (var i = 0; i < allRows.length; i++) {
      ...
      // Deal with note rows which will never be the very first row
        ...
        if (
          element.getAttribute("data-state") == "expanded" &&
          $(element).prev().hasClass("visible")
        ) {
          $(element).addClass("visible");
    ...
    alert(
      "Can't set row visibility. Unable to run this in your browser, sorry.",
    );
  }
}
    ...
    for (var i = 0; i < siblingCount; i++) {
      ...
      if ($(element).next()[0] != null) {
        if ($(element).next().hasClass("visible")) {
          ...
          if (
            $(element).next().attr("data-level") < $(this).attr("data-level")
          ) {
...
function setWidthsForSectionLevel(startRow) {
  ...
  while (stillInSection == true) {
    ...
    var thisDataLevel =
      nextRow != null
        ? Number(currentRow.nextElementSibling.getAttribute("data-level"))
        : null;
...
function setOutlineTitleInset() {
  ...
  if (outlineTitleDiv != null) {
    var firstRowOutlineCells = firstRow
      .querySelector("table.outline-column")
